...
export class Trap extends Phaser.GameObjects.Rectangle {
    constructor(scene, config) {
        ...
        this.movementActivated =
            !config.triggerDistance || this.type !== "movingHazard";
    ...
    createAlienVisuals() {
        ...
        this.tip = this.scene.add
            ...
            .setDepth(172);
        this.fangL = this.scene.add
            .rectangle(
                this.x - 2.6,
                this.y - this.height * 0.29,
                1.4,
                3.4,
                p.bio,
                0.9,
    ...
    syncVisual() {
        ...
        const flutter = Math.sin(
            this.scene.time.now * 0.014 + this.startX * 0.01,
        );
        ...
        if (this.type === "movingHazard") {
            this.glow?.setPosition(
                this.x,
                baseY + this.height * 0.22 + flutter * 0.5,
            );
        ...
        this.jawL.setPosition(
            this.x - this.width * 0.2,
            baseY - this.height * 0.42,
        );
        this.jawL.setAngle(-8 + flap * 7);
        this.jawR.setPosition(
            this.x + this.width * 0.2,
            baseY - this.height * 0.42,
        );
        this.jawR.setAngle(8 - flap * 7);
        this.tip.setPosition(this.x, baseY - this.height * 0.55);
        this.fangL.setPosition(
            this.x - 2.6,
            baseY - this.height * 0.29 + flutter * 0.2,
        );
        this.fangR.setPosition(
            this.x + 2.6,
            baseY - this.height * 0.29 - flutter * 0.2,
        );
    }
    ...
    update(player) {
        ...
        if (this.type !== "movingHazard") {
            this.wobbleOffset =
                Math.sin(this.scene.time.now * 0.012 + this.startX * 0.02) *
                0.9;
        }
